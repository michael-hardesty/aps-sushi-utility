...
async function processWithProgress(requestData, writer, encoder) {
  ...
  // Send final results
  const csv = convertToCSV(results, requestData)
  ...
  const finalResult = {
    type: 'complete',
    csv,
    filename: generateFilename(requestData), // Add filename
    successful,
    failed,
...
function buildApiUrl(requestData, account) {
  ...
  const baseUrl = requestData.format === '5.1'
    ? 'https://journals.physiology.org/r51/reports'
    : 'https://journals.physiology.org/reports'
